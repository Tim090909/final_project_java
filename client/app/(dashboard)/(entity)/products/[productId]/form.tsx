...
const ProductForm = ({product, categories} : FormProps) => {
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        try{
            const response = await axios.post(`${API_URL}/api/products/${product.id}`, values);
  ...
  return (
    <div className=" max-w-5xl mx-auto flex md:items-center md:justify-center h-full p-6 mt-16">
      <div>
        ...
        <p className="text-sm text-slate-400">Edit title, category and description of product!</p>
        <Form {...form}>
            <form
